...
class Program
{
    ...
    //class MyTaskV2<TResult> : MyTask 
    //{
    //    TResult result;
    //    public TResult Result { get { Wait(); return result; } }

    //    public MyTaskV2(Func<TResult> action) : base(() => { result = action.Invoke(); })
    //    {

    //    }

    //    public override void ContinueWith(Action action)
    //    {
    //        base.ContinueWith(action);
    ...
    readonly struct MyCancellationToken : IEquatable<MyCancellationToken> 
    {
    ...
    public static async Task<string> ReadContent(Uri uri)
    {
    ...
    public static async Task HowToUseAwait()
    {
        var task = new HttpClient().GetStringAsync("https://httpbin.org/anything");
    ...
    public static Task<string> SimulateHttpRequest(string Url) => new HttpClient().GetStringAsync(Url);
    public static async Task<string> SimulateHttpRequestAsync(string Url) => await new HttpClient().GetStringAsync(Url);

    public static void DownloadOldStyle()
    {
        string url = "https://httpbin.org/anything";
        SimulateHttpRequest(url).ContinueWith(t =>
        {
            Console.WriteLine($"Response recieved : {t.Result} ");
        });

        int count = TaskExamples.CountPrimeNumberInRange(1, 1_000_000);

        Console.WriteLine(count);

    }

    public async static Task DownloadNewStyle()
    {

        string url = "https://httpbin.org/anything";
        var task = SimulateHttpRequest(url); // returns back the control to caller until the task finish

        int count = TaskExamples.CountPrimeNumberInRange(1, 1_000_000);
        Console.WriteLine(count);

        string content = await task;
        Console.WriteLine($"Response recieved : {content}");
    }
    ...
    public static async Task FormatResponseAsync(string Url)
    {
    ...
    public static void FormatResponseAsync_NoAwait(string Url)
    {
    ...
    public static async Task DownloadGameAsync(CancellationToken token)
    {
        try
        {
            await Task.Delay(TimeSpan.FromSeconds(1000) , token);
        }
        catch (Exception ex)
        {
    ...
    public static async Task Main()
    {
        ...
        await ReportProgressAsync(100, cts.Token, new Progress<int> ((n) => { Console.WriteLine($"{n} instaling..."); }));
        ...
        //var task = Cancella;
            ...
            //token1.Register(() =>
            //{
            //    Console.WriteLine("Registered 1");
            //});
            //token1.Register(() =>
            //{
            //    Console.WriteLine("Registered 2 ");
            //});

            //for (int i = 0; i < 10; i++)
            //{
            //    var task = CancellationTokenExamples.StillAlive(token1);
            //}
...
// old way using asynchronous programming 
...
//var task = Task.Run<string>(() => client.GetStringAsync(url2));
//task.ContinueWith((task) => Console.WriteLine($"Task Finished\nResult : {task.Result}"));

//Thread.Sleep(5000); // processing things
